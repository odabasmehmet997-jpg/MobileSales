...
class EnumTypeAdapter<T extends Enum<T>> extends TypeAdapter<T> {
  static final TypeAdapterFactory FACTORY =
      new TypeAdapterFactory() {
        ...
        public <T> TypeAdapters.EnumTypeAdapter create(Gson gson, TypeToken<T> typeToken) {
          ...
          if (!Enum.class.isAssignableFrom(rawType) || Enum.class == rawType) {
            return null;
          }
          if (!rawType.isEnum()) {
            rawType = rawType.getSuperclass(); // handle anonymous subclasses
          }
          @SuppressWarnings({"rawtypes", "unchecked"}) TypeAdapter<T> adapter = (TypeAdapter<T>) new EnumTypeAdapter(rawType);
          return adapter;
        }
      };
  ...
  public T read(JsonReader in) throws IOException {
    ...
    return (null == constant) ? stringToConstant.get(key) : constant;
  }
