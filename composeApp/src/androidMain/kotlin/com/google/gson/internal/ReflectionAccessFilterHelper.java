...
public enum ReflectionAccessFilterHelper {
    ;
   ...
   * ReflectionAccessFilter.FilterResult#INDECISIVE}, or {@link ReflectionAccessFilter.FilterResult#ALLOW} if the list of filters is empty or all
   * returned {@code INDECISIVE}.
   */
  public static ReflectionAccessFilter.FilterResult getFilterResult(
          final List<ReflectionAccessFilter> reflectionFilters, final Class<?> c) {
    for (final ReflectionAccessFilter filter : reflectionFilters) {
      final ReflectionAccessFilter.FilterResult result = filter.check(c);
      if (ReflectionAccessFilter.FilterResult.INDECISIVE != result) {
        return result;
      }
    }
