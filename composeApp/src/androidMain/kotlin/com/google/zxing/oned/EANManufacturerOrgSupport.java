...
final class EANManufacturerOrgSupport {
    ...
    public String lookupCountryIdentifier(final String str) {
        int[] iArr;
        int i2;
        this.initIfNeeded();
        final int parseInt = Integer.parseInt(str.substring(0, 3));
        final int size = ranges.size();
        for (int i3 = 0; i3 < size && parseInt >= (i2 = iArr[0]); i3++) {
            if (1 != (iArr = this.ranges.get(i3)).length) {
                i2 = iArr[1];
            }
            if (parseInt <= i2) {
                return countryIdentifiers.get(i3);
            }
        }
        return null;
    }

    private void add(final int[] iArr, final String str) {
        ranges.add(iArr);
        countryIdentifiers.add(str);
    }
    ...
    private synchronized void initIfNeeded() {
        if (ranges.isEmpty()) {
            ...
            this.add(new int[]{MaterialCardViewHelper.DEFAULT_FADE_ANIM_DURATION, 379}, "FR");
            ...
            this.add(new int[]{TypedValues.MotionType.TYPE_EASING}, "GH");
            this.add(new int[]{TypedValues.MotionType.TYPE_DRAW_PATH}, "BH");
            this.add(new int[]{TypedValues.MotionType.TYPE_POLAR_RELATIVETO}, "MU");
            this.add(new int[]{TypedValues.MotionType.TYPE_QUANTIZE_INTERPOLATOR_TYPE}, "MA");
            ...
            this.add(new int[]{TypedValues.TransitionType.TYPE_DURATION, 709}, "NO");
            ...
            this.add(new int[]{TypedValues.Custom.TYPE_INT, 919}, "AT");
