...
public class CreatorCollector {
    ...
    public void setDefaultCreator(AnnotatedWithParams annotatedWithParams) {
        this._creators[0] = _fixAccess(annotatedWithParams);
    }
    ...
    public void addDelegatingCreator(AnnotatedWithParams annotatedWithParams, boolean z, SettableBeanProperty[] settableBeanPropertyArr, int r4) {
        if (annotatedWithParams.getParameterType(r4).isCollectionLikeType()) {
        ...
        if (verifyNonDup(annotatedWithParams, 9, z)) {
            if (settableBeanPropertyArr.length > 1) {
                ...
                for (int r0 = 0; r0 < length; r0++) {
                    String name = settableBeanPropertyArr[r0].getName();
                    if ((!name.isEmpty() || settableBeanPropertyArr[r0].getInjectableValueId() == null) && (num = (Integer) map.put(name, Integer.valueOf(r0))) != null) {
                        throw new IllegalArgumentException(String.format("Duplicate creator property \"%s\" (index %s vs %d) for type %s ", name, num, Integer.valueOf(r0), ClassUtil.nameOf(this._beanDesc.getBeanClass())));
                    }
                }
            }
    ...
    private JavaType _computeDelegateType(DeserializationContext deserializationContext, AnnotatedWithParams annotatedWithParams, SettableBeanProperty[] settableBeanPropertyArr) throws JsonMappingException {
        ...
        if (settableBeanPropertyArr != null) {
            ...
            while (true) {
                ...
                if (settableBeanPropertyArr[r1] == null) {
                    r3 = r1;
                    break;
                }
                r1++;
        ...
        DeserializationConfig config = deserializationContext.getConfig();
        JavaType parameterType = annotatedWithParams.getParameterType(r3);
        AnnotationIntrospector annotationIntrospector = config.getAnnotationIntrospector();
        if (annotationIntrospector == null) {
            return parameterType;
        }
        AnnotatedParameter parameter = annotatedWithParams.getParameter(r3);
    ...
    protected boolean verifyNonDup(AnnotatedWithParams annotatedWithParams, int r9, boolean z) {
        boolean z2;
        int r1 = 1 << r9;
        this._hasNonDefaultCreator = true;
        AnnotatedWithParams annotatedWithParams2 = this._creators[r9];
            ...
            if (z2 && annotatedWithParams2.getClass() == annotatedWithParams.getClass()) {
                ...
                if (rawParameterType == rawParameterType2) {
                    ...
                    if (!_isEnumValueOf(annotatedWithParams2)) {
                        _reportDuplicateCreator(r9, z, annotatedWithParams2, annotatedWithParams);
                    }
                } else {
        ...
        this._creators[r9] = _fixAccess(annotatedWithParams);
        return true;
    }
    protected void _reportDuplicateCreator(int r2, boolean z, AnnotatedWithParams annotatedWithParams, AnnotatedWithParams annotatedWithParams2) {
        throw new IllegalArgumentException(String.format("Conflicting %s creators: already had %s creator %s, encountered another: %s", TYPE_DESCS[r2], z ? "explicitly marked" : "implicitly discovered", annotatedWithParams, annotatedWithParams2));
    }
