...
public class JobInfoScheduler implements WorkScheduler {
    ...
    int getJobId(TransportContext transportContext) {
        ...
        adler32.update(this.context.getPackageName().getBytes(StandardCharsets.UTF_8));
        adler32.update(transportContext.getBackendName().getBytes(StandardCharsets.UTF_8));
    ...
    public void schedule(TransportContext transportContext, int i2, boolean z) {
        ...
        Logging.d("JobInfoScheduler", "Scheduling upload for context %s with jobId=%d in %dms(Backend next call timestamp %d). Attempt %d", transportContext, Integer.valueOf(jobId), Long.valueOf(this.config.getScheduleDelay(transportContext.getPriority(), nextCallTime, i2)), Long.valueOf(nextCallTime), Integer.valueOf(i2));
