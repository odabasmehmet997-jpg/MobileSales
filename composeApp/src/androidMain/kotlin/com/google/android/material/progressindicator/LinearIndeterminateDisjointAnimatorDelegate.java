...
final class LinearIndeterminateDisjointAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private void updateSegmentPositions(int i2) {
        for (int i3 = 0; i3 < this.activeIndicators.size(); i3++) {
            ...
            activeIndicator.startFraction = MathUtils.clamp(this.interpolatorArray[i4].getInterpolation(getFractionInRange(i2, i5, iArr2[i4])), 0.0f, 1.0f);
            int i6 = i4 + 1;
            activeIndicator.endFraction = MathUtils.clamp(this.interpolatorArray[i6].getInterpolation(getFractionInRange(i2, iArr[i6], iArr2[i6])), 0.0f, 1.0f);
        }
    }
