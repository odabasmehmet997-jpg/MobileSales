...
final class CircularIndeterminateAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private void updateSegmentPositions(int i2) {
        ...
        for (int i3 = 0; i3 < 4; i3++) {
            activeIndicator.endFraction += this.interpolator.getInterpolation(getFractionInRange(i2, DELAY_TO_EXPAND_IN_MS[i3], 667)) * 250.0f;
            activeIndicator.startFraction += this.interpolator.getInterpolation(getFractionInRange(i2, DELAY_TO_COLLAPSE_IN_MS[i3], 667)) * 250.0f;
        }
        float f3 = activeIndicator.startFraction;
        float f4 = activeIndicator.endFraction;
        activeIndicator.startFraction = (f3 + ((f4 - f3) * this.completeEndFraction)) / 360.0f;
