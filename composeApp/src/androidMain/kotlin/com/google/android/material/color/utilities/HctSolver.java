...
public class HctSolver {
    ...
    static double[] setCoordinate(double[] dArr, double d2, double[] dArr2, int i2) {
        return lerpPoint(dArr, intercept(dArr[i2], d2, dArr2[i2]), dArr2);
    }

    static double[] nthVertex(double d2, int i2) {
        ...
        for (int i2 = 0; i2 < 12; i2++) {
            ...
            if (dArrNthVertex[0] >= 0.0d) {
                ...
                if (!z) {
                    ...
                    d4 = dHueOf;
                    d5 = d4;
                } else if (z2 || areInCyclicOrder(d4, dHueOf, d5)) {
    ...
    static int findResultByJ(double d2, double d3, double d4) {
        ...
        while (i2 < 5) {
            ...
            double d7 = dCos;
            double dPow2 = Math.pow(((d3 == 0.0d || dSqrt == 0.0d) ? 0.0d : d3 / Math.sqrt(d6)) * dPow, 1.1111111111111112d);
            double aw = (viewingConditions.getAw() * Math.pow(d6, (d5 / viewingConditions.getC()) / viewingConditions.getZ())) / viewingConditions.getNbb();
            double d8 = (((0.305d + aw) * 23.0d) * dPow2) / (((23.0d * d7) + ((dPow2 * 11.0d) * dCos2)) + ((dPow2 * 108.0d) * dSin));
            double d9 = d8 * dCos2;
            double d10 = d8 * dSin;
            double d11 = aw * 460.0d;
            double[] dArrMatrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation((((451.0d * d9) + d11) + (288.0d * d10)) / 1403.0d), inverseChromaticAdaptation(((d11 - (891.0d * d9)) - (261.0d * d10)) / 1403.0d), inverseChromaticAdaptation(((d11 - (d9 * 220.0d)) - (d10 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            ...
            double[] dArr = Y_FROM_LINRGB;
            double d15 = (dArr[0] * d12) + (dArr[1] * d13) + (dArr[2] * d14);
            if (d15 <= 0.0d) {
                return 0;
            }
            if (i2 != 4) {
