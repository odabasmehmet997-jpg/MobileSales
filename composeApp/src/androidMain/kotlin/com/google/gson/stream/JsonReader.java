...
public class JsonReader implements Closeable {
  ...
  public final boolean isLenient() {
    return Strictness.LENIENT == this.strictness;
  }
   ...
   * allows reading the JSON data {@code [{"a":[true]}]}, but for a nesting limit of 2 it would
  ...
  public boolean hasNext() throws IOException {
    ...
    return PEEKED_END_OBJECT != p && PEEKED_END_ARRAY != p && PEEKED_EOF != p;
  }
  ...
  int doPeek() throws IOException {
    ...
    if (JsonScope.EMPTY_ARRAY == peekStack) {
        stack[stackSize - 1] = JsonScope.NONEMPTY_ARRAY;
    } else if (JsonScope.NONEMPTY_ARRAY == peekStack) {
    ...
    } else if (JsonScope.EMPTY_OBJECT == peekStack || JsonScope.NONEMPTY_OBJECT == peekStack) {
        stack[stackSize - 1] = JsonScope.DANGLING_NAME;
      // Look for a comma before the next element.
      if (JsonScope.NONEMPTY_OBJECT == peekStack) {
        int c = nextNonWhitespace(true);
        switch (c) {
          case '}':
    ...
    } else if (JsonScope.DANGLING_NAME == peekStack) {
      ...
      switch (c) {
        ...
        case '=':
            checkLenient();
          if ((pos < limit || fillBuffer(1)) && '>' == this.buffer[this.pos]) {
              pos++;
          }
          break;
        default:
    ...
    } else if (JsonScope.EMPTY_DOCUMENT == peekStack) {
      if (Strictness.LENIENT == this.strictness) {
          consumeNonExecutePrefix();
      }
        stack[stackSize - 1] = JsonScope.NONEMPTY_DOCUMENT;
    } else if (JsonScope.NONEMPTY_DOCUMENT == peekStack) {
    ...
    } else if (JsonScope.CLOSED == peekStack) {
    ...
    switch (c) {
      ...
      case ',':
        ...
        if (JsonScope.EMPTY_ARRAY == peekStack || JsonScope.NONEMPTY_ARRAY == peekStack) {
            checkLenient();
            pos--;
            peeked = PEEKED_NULL;
          return peeked;
  ...
  private int peekKeyword() throws IOException {
    ...
    boolean allowsUpperCased = Strictness.STRICT != this.strictness;
  ...
  private int peekNumber() throws IOException {
    ...
    for (; true; i++) {
      ...
      switch (c) {
        case '-':
          if (NUMBER_CHAR_NONE == last) {
            negative = true;
            last = NUMBER_CHAR_SIGN;
            continue;
          } else if (NUMBER_CHAR_EXP_E == last) {
        ...
        case 'e':
        case 'E':
          if (NUMBER_CHAR_DIGIT == last || NUMBER_CHAR_FRACTION_DIGIT == last) {
        ...
        default:
          ...
          if (NUMBER_CHAR_SIGN == last || NUMBER_CHAR_NONE == last) {
            value = -(c - '0');
            last = NUMBER_CHAR_DIGIT;
          } else if (NUMBER_CHAR_DIGIT == last) {
            if (0 == value) {
              return PEEKED_NONE; // Leading '0' prefix is not allowed (since it could be octal).
            }
            long newValue = value * 10 - (c - '0');
            fitsInLong &=
                    MIN_INCOMPLETE_INTEGER < value
                    || (MIN_INCOMPLETE_INTEGER == value && newValue < value);
            value = newValue;
          } else if (NUMBER_CHAR_DECIMAL == last) {
            last = NUMBER_CHAR_FRACTION_DIGIT;
          } else if (NUMBER_CHAR_EXP_E == last || NUMBER_CHAR_EXP_SIGN == last) {
    ...
    if (NUMBER_CHAR_DIGIT == last
      ...
      return peeked;
        ...
        || NUMBER_CHAR_FRACTION_DIGIT == last
        || NUMBER_CHAR_EXP_DIGIT == last) {
  ...
  public String nextName() throws IOException {
    ...
    String result;
    if (PEEKED_UNQUOTED_NAME == p) {
      result = nextUnquotedValue();
    } else if (PEEKED_SINGLE_QUOTED_NAME == p) {
      result = nextQuotedValue('\'');
    } else if (PEEKED_DOUBLE_QUOTED_NAME == p) {
      result = nextQuotedValue('"');
    } else {
  ...
  public String nextString() throws IOException {
    ...
    String result;
    if (PEEKED_UNQUOTED == p) {
      result = nextUnquotedValue();
    } else if (PEEKED_SINGLE_QUOTED == p) {
      result = nextQuotedValue('\'');
    } else if (PEEKED_DOUBLE_QUOTED == p) {
      result = nextQuotedValue('"');
    } else if (PEEKED_BUFFERED == p) {
      result = peekedString;
        peekedString = null;
    } else if (PEEKED_LONG == p) {
      result = Long.toString(peekedLong);
    } else if (PEEKED_NUMBER == p) {
  ...
  public double nextDouble() throws IOException {
    ...
    if (PEEKED_NUMBER == p) {
        ...
        pos += peekedNumberLength;
    } else if (PEEKED_SINGLE_QUOTED == p || PEEKED_DOUBLE_QUOTED == p) {
        peekedString = nextQuotedValue(PEEKED_SINGLE_QUOTED == p ? '\'' : '"');
    } else if (PEEKED_UNQUOTED == p) {
    ...
    if (Strictness.LENIENT != this.strictness && (Double.isNaN(result) || Double.isInfinite(result))) {
  ...
  public long nextLong() throws IOException {
    ...
    if (PEEKED_NUMBER == p) {
        peekedString = new String(buffer, pos, peekedNumberLength);
        pos += peekedNumberLength;
    } else if (PEEKED_SINGLE_QUOTED == p || PEEKED_DOUBLE_QUOTED == p || PEEKED_UNQUOTED == p) {
      if (PEEKED_UNQUOTED == p) {
          peekedString = nextUnquotedValue();
      } else {
          peekedString = nextQuotedValue(PEEKED_SINGLE_QUOTED == p ? '\'' : '"');
  ...
  private String nextQuotedValue(char quote) throws IOException {
    ...
    while (true) {
      ...
      while (p < l) {
        ...
        // through U+001F)
        if (Strictness.STRICT == this.strictness && 0x20 > c) {
          throw syntaxError(
  ...
  private boolean fillBuffer(int minimum) throws IOException {
    ...
    if (limit != pos) {
      ...
      pos = 0;
    int total;
    while (-1 != (total = this.in.read(buffer, this.limit, buffer.length - this.limit))) {
        limit += total;

      // if this is the first read, consume an optional byte order mark (BOM) if it exists
      if (0 == this.lineNumber && 0 == this.lineStart && 0 < this.limit && '\ufeff' == buffer[0]) {
          pos++;
          lineStart++;
        minimum++;
      }
    ...
    for (int i = 0; i < stackSize; i++) {
      ...
      switch (scope) {
        ...
        case JsonScope.NONEMPTY_ARRAY:
          ...
          if (usePreviousPath && 0 < pathIndex && i == stackSize - 1) {
            pathIndex--;
          }
  ...
  private char readEscapeCharacter() throws IOException {
    ...
    char escaped = buffer[pos];
      ...
      case '\n':
        if (Strictness.STRICT == this.strictness) {
  ...
  private IllegalStateException unexpectedTokenError(String expected) throws IOException {
    ...
    String troubleshootingId =
            JsonToken.NULL == peeked ? "adapter-not-null-safe" : "unexpected-json-structure";
    ...
    JsonReaderInternalAccess.INSTANCE =
        new JsonReaderInternalAccess() {
          ...
          public void promoteNameToValue(JsonReader reader) throws IOException {
            ...
            if (PEEKED_DOUBLE_QUOTED_NAME == p) {
              reader.peeked = PEEKED_DOUBLE_QUOTED;
            } else if (PEEKED_SINGLE_QUOTED_NAME == p) {
              reader.peeked = PEEKED_SINGLE_QUOTED;
            } else if (PEEKED_UNQUOTED_NAME == p) {
