    ...
    static final class Builder {
        ...
        RequestFactory build() {
            ...
            if (this.httpMethod == null) {
                throw Utils.methodError(this.method, "HTTP method annotation is required (e.g., @GET, @POST, etc.).");
            }
            if (!this.hasBody) {
                if (this.isMultipart) {
                    throw Utils.methodError(this.method, "Multipart can only be specified on HTTP methods with request body (e.g., @POST).");
                }
                if (this.isFormEncoded) {
                    throw Utils.methodError(this.method, "FormUrlEncoded can only be specified on HTTP methods with request body (e.g., @POST).");
                }
            }
            ...
            if (!z && !this.isMultipart && !this.hasBody && this.gotBody) {
                throw Utils.methodError(this.method, "Non-body HTTP method cannot contain @Body.");
            }
            if (z && !this.gotField) {
                throw Utils.methodError(this.method, "Form-encoded method must contain at least one @Field.");
            }
            if (this.isMultipart && !this.gotPart) {
                throw Utils.methodError(this.method, "Multipart method must contain at least one @Part.");
            }
        ...
        private void parseMethodAnnotation(Annotation annotation) {
            ...
            if (annotation instanceof HTTP http) {
            ...
            if (annotation instanceof retrofit2.http.Headers) {
                ...
                if (strArrValue.length == 0) {
                    throw Utils.methodError(this.method, "@Headers annotation is empty.");
                }
            ...
            if (annotation instanceof Multipart) {
                if (this.isFormEncoded) {
                    throw Utils.methodError(this.method, "Only one encoding annotation is allowed.");
                }
        ...
        private ParameterHandler<?> parseParameter(int i2, Type type, Annotation[] annotationArr, boolean z) {
                ...
                for (Annotation annotation : annotationArr) {
                    ...
                    if (parameterAnnotation != null) {
                        if (parameterHandler != null) {
                            throw Utils.parameterError(this.method, i2, "Multiple Retrofit annotations found, only one allowed.");
                        }
            ...
            throw Utils.parameterError(this.method, i2, "No Retrofit annotation found.");
        }
        ...
        private ParameterHandler<?> parseParameterAnnotation(int i2, Type type, Annotation[] annotationArr, Annotation annotation) {
            if (annotation instanceof Url) {
                ...
                if (this.gotUrl) {
                    throw Utils.parameterError(this.method, i2, "Multiple @Url method annotations found.");
                }
                if (this.gotPath) {
                    throw Utils.parameterError(this.method, i2, "@Path parameters may not be used with @Url.");
                }
                if (this.gotQuery) {
                    throw Utils.parameterError(this.method, i2, "A @Url parameter must not come after a @Query.");
                }
                if (this.gotQueryName) {
                    throw Utils.parameterError(this.method, i2, "A @Url parameter must not come after a @QueryName.");
                }
                if (this.gotQueryMap) {
                    throw Utils.parameterError(this.method, i2, "A @Url parameter must not come after a @QueryMap.");
                }
                ...
                throw Utils.parameterError(this.method, i2, "@Url must be okhttp3.HttpUrl, String, java.net.URI, or android.net.Uri type.");
            }
            if (annotation instanceof Path path) {
                validateResolvableType(i2, type);
                if (this.gotQuery) {
                    throw Utils.parameterError(this.method, i2, "A @Path parameter must not come after a @Query.");
                }
                if (this.gotQueryName) {
                    throw Utils.parameterError(this.method, i2, "A @Path parameter must not come after a @QueryName.");
                }
                if (this.gotQueryMap) {
                    throw Utils.parameterError(this.method, i2, "A @Path parameter must not come after a @QueryMap.");
                }
                if (this.gotUrl) {
                    throw Utils.parameterError(this.method, i2, "@Path parameters may not be used with @Url.");
                }
            ...
            if (annotation instanceof Query query) {
                ...
                if (Iterable.class.isAssignableFrom(rawType)) {
                    if (!(type instanceof ParameterizedType)) {
                        throw Utils.parameterError(this.method, i2, rawType.getSimpleName() + " must include generic type (e.g., " + rawType.getSimpleName() + "<String>)");
                    }
            ...
            if (annotation instanceof QueryName) {
                ...
                if (Iterable.class.isAssignableFrom(rawType2)) {
                    if (!(type instanceof ParameterizedType)) {
                        throw Utils.parameterError(this.method, i2, rawType2.getSimpleName() + " must include generic type (e.g., " + rawType2.getSimpleName() + "<String>)");
                    }
            ...
            if (annotation instanceof QueryMap) {
                ...
                if (!Map.class.isAssignableFrom(rawType3)) {
                    throw Utils.parameterError(this.method, i2, "@QueryMap parameter type must be Map.");
                }
                Type supertype = Utils.getSupertype(type, rawType3, Map.class);
                if (!(supertype instanceof ParameterizedType parameterizedType)) {
                    throw Utils.parameterError(this.method, i2, "Map must include generic types (e.g., Map<String, String>)");
                }
                Type parameterUpperBound = Utils.getParameterUpperBound(0, parameterizedType);
                if (String.class != parameterUpperBound) {
                    throw Utils.parameterError(this.method, i2, "@QueryMap keys must be of type String: " + parameterUpperBound);
                }
            ...
            if (annotation instanceof Header) {
                ...
                if (Iterable.class.isAssignableFrom(rawType4)) {
                    if (!(type instanceof ParameterizedType)) {
                        throw Utils.parameterError(this.method, i2, rawType4.getSimpleName() + " must include generic type (e.g., " + rawType4.getSimpleName() + "<String>)");
                    }
            ...
            if (annotation instanceof HeaderMap) {
                ...
                if (!Map.class.isAssignableFrom(rawType5)) {
                    throw Utils.parameterError(this.method, i2, "@HeaderMap parameter type must be Map.");
                }
                Type supertype2 = Utils.getSupertype(type, rawType5, Map.class);
                if (!(supertype2 instanceof ParameterizedType parameterizedType2)) {
                    throw Utils.parameterError(this.method, i2, "Map must include generic types (e.g., Map<String, String>)");
                }
                Type parameterUpperBound2 = Utils.getParameterUpperBound(0, parameterizedType2);
                if (String.class != parameterUpperBound2) {
                    throw Utils.parameterError(this.method, i2, "@HeaderMap keys must be of type String: " + parameterUpperBound2);
                }
            ...
            if (annotation instanceof Field field) {
                validateResolvableType(i2, type);
                if (!this.isFormEncoded) {
                    throw Utils.parameterError(this.method, i2, "@Field parameters can only be used with form encoding.");
                }
                ...
                if (Iterable.class.isAssignableFrom(rawType6)) {
                    if (!(type instanceof ParameterizedType)) {
                        throw Utils.parameterError(this.method, i2, rawType6.getSimpleName() + " must include generic type (e.g., " + rawType6.getSimpleName() + "<String>)");
                    }
            ...
            if (annotation instanceof FieldMap) {
                validateResolvableType(i2, type);
                if (!this.isFormEncoded) {
                    throw Utils.parameterError(this.method, i2, "@FieldMap parameters can only be used with form encoding.");
                }
                Class<?> rawType7 = Utils.getRawType(type);
                if (!Map.class.isAssignableFrom(rawType7)) {
                    throw Utils.parameterError(this.method, i2, "@FieldMap parameter type must be Map.");
                }
                Type supertype3 = Utils.getSupertype(type, rawType7, Map.class);
                if (!(supertype3 instanceof ParameterizedType parameterizedType3)) {
                    throw Utils.parameterError(this.method, i2, "Map must include generic types (e.g., Map<String, String>)");
                }
                Type parameterUpperBound3 = Utils.getParameterUpperBound(0, parameterizedType3);
                if (String.class != parameterUpperBound3) {
                    throw Utils.parameterError(this.method, i2, "@FieldMap keys must be of type String: " + parameterUpperBound3);
                }
            ...
            if (annotation instanceof Part part) {
                validateResolvableType(i2, type);
                if (!this.isMultipart) {
                    throw Utils.parameterError(this.method, i2, "@Part parameters can only be used with multipart encoding.");
                }
                this.gotPart = true;
                ...
                if (strValue5.isEmpty()) {
                    if (Iterable.class.isAssignableFrom(rawType8)) {
                        if (!(type instanceof ParameterizedType)) {
                            throw Utils.parameterError(this.method, i2, rawType8.getSimpleName() + " must include generic type (e.g., " + rawType8.getSimpleName() + "<String>)");
                        }
                        if (!MultipartBody.Part.class.isAssignableFrom(Utils.getRawType(Utils.getParameterUpperBound(0, (ParameterizedType) type)))) {
                            throw Utils.parameterError(this.method, i2, "@Part annotation must supply a name or use MultipartBody.Part parameter type.");
                        }
                ...
                if (Iterable.class.isAssignableFrom(rawType8)) {
                    ...
                    if (MultipartBody.Part.class.isAssignableFrom(Utils.getRawType(parameterUpperBound4))) {
                        throw Utils.parameterError(this.method, i2, "@Part parameters using the MultipartBody.Part must not include a part name in the annotation.");
                    }
            ...
            if (annotation instanceof PartMap) {
                validateResolvableType(i2, type);
                if (!this.isMultipart) {
                    throw Utils.parameterError(this.method, i2, "@PartMap parameters can only be used with multipart encoding.");
                }
                this.gotPart = true;
                Class<?> rawType9 = Utils.getRawType(type);
                if (!Map.class.isAssignableFrom(rawType9)) {
                    throw Utils.parameterError(this.method, i2, "@PartMap parameter type must be Map.");
                }
                Type supertype4 = Utils.getSupertype(type, rawType9, Map.class);
                if (!(supertype4 instanceof ParameterizedType parameterizedType4)) {
                    throw Utils.parameterError(this.method, i2, "Map must include generic types (e.g., Map<String, String>)");
                }
                Type parameterUpperBound5 = Utils.getParameterUpperBound(0, parameterizedType4);
                if (String.class != parameterUpperBound5) {
                    throw Utils.parameterError(this.method, i2, "@PartMap keys must be of type String: " + parameterUpperBound5);
                }
                ...
                if (MultipartBody.Part.class.isAssignableFrom(Utils.getRawType(parameterUpperBound6))) {
                    throw Utils.parameterError(this.method, i2, "@PartMap values cannot be MultipartBody.Part. Use @Part List<Part> or a different value type instead.");
                }
            ...
            if (annotation instanceof Body) {
                ...
                if (this.isFormEncoded || this.isMultipart) {
                    throw Utils.parameterError(this.method, i2, "@Body parameters cannot be used with form or multi-part encoding.");
                }
                if (this.gotBody) {
                    throw Utils.parameterError(this.method, i2, "Multiple @Body method annotations found.");
                }
                try {
            ...
            for (int i3 = i2 - 1; i3 >= 0; i3--) {
                ...
                if ((parameterHandler instanceof ParameterHandler.Tag) && ((ParameterHandler.Tag) parameterHandler).cls.equals(rawType10)) {
                    throw Utils.parameterError(this.method, i2, "@Tag type " + rawType10.getName() + " is duplicate of parameter #" + (i3 + 1) + " and would always overwrite its value.");
                }
            }
